...
apiInput = input("Please give the Input to generate a LinkedIn post with GIF? ")
...
# Calling OpenAI's DALL-E API with required input parameters to get the GIF url
openai.api_key = os.getenv("OPENAI_API_KEY")

# DALL-E API, which takes the prompt as the input and then it generates it into random GIF
response = openai.Image.create(
    prompt="Random GIF of a " + chain.run(apiInput),
    n=1,
    size="256x256",
    response_format="url",
)
...
# 2. From the response, we get the GIF url
# 3. And then, just combine it, so that it looks like a LinkedIn post with a GIF!
